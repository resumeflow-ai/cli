"""
ResumeFlow CLI - Main command-line interface.

Commands:
    analyze     Analyze repositories and generate profiles (auto-pushed)
    push        Push local profiles to resumeflow.dev
    view        View the latest profile
    login       Authenticate with ResumeFlow
...
def version_callback(value: bool):
    if value:
        console.print(f"ResumeFlow CLI v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """ResumeFlow CLI - Privacy-first developer profile generator."""
    pass
...
def analyze(
    ...
    and pushed to resumeflow.dev immediately. Process halts on any failure.
    
    Examples:
        rf analyze ~/code                              # OpenAI (default)
        rf analyze ~/code --local --model llama3.2     # Local LLM (Ollama)
        rf analyze ~/code --offline                    # Stats only, no push
    """
    print_header()
    ...
    if mode != "offline":
        if not is_authenticated():
            print_error("Not logged in. Please run 'rf login' first.")
            raise typer.Exit(1)
    ...
    for i, repo in enumerate(filtered_repos, 1):
        ...
        if mode != "offline":
            try:
                ...
                console.print(f"  [green]Pushed to resumeflow.dev[/green]")
    ...
    console.print()
    print_next_steps(["Run [bold]rf profiles[/] to see all saved profiles"])
...
def _generate_single_repo_offline_profile(repo: RepoInfo, verbose: bool = False) -> str:
    ...
    lines.append("---")
    lines.append("*Generated by ResumeFlow CLI (offline mode)*")
...
def view(
    ...
    Example:
        rf view
        rf view --profile myrepo_2025-12-26
    ...
    if profile_name:
        ...
        if not profile_path:
            ...
            print_info("Run 'rf analyze <paths>' to generate a profile.")
            raise typer.Exit(1)
...
def login():
    """
    Authenticate with ResumeFlow using device code flow.
    ...
    console.print("[bold]🔐  Authenticate with ResumeFlow[/]")
    console.print()
...
def profiles():
    ...
    console.print(f"  rf view --profile {saved_profiles[0]['name']}")
    if not saved_profiles[0]["synced"]:
        console.print(f"  rf push --profile {saved_profiles[0]['name']}")
...
def push(
    ...
    """
    Push local profiles to resumeflow.dev.
    
    By default, pushes all unsynced profiles. Use --profile to push a specific one.
    
    Examples:
        rf push                              # Push all unsynced profiles
        rf push --profile myrepo_2025-12-26  # Push specific profile
    """
    if not is_authenticated():
        print_error("Not logged in. Please run 'rf login' first.")
        raise typer.Exit(1)
    ...
    if not all_profiles:
        print_info("No profiles found.")
        print_info("Run 'rf analyze <paths>' to generate profiles first.")
        raise typer.Exit()
    ...
    console.print(f"Pushing {len(profiles_to_push)} profile(s) to resumeflow.dev...")
    console.print()
    ...
    if pushed_count > 0 and failed_count == 0:
        console.print()
        print_next_steps([
            "View your profiles at https://resumeflow.dev/profile",
            "Run [bold]rf analyze <paths>[/] to update your profiles",
        ])
...
def show_config():
    ...
    console.print("[bold]ResumeFlow CLI Configuration[/]")
    console.print()
    ...
    console.print("  RESUMEFLOW_DEV=1         Enable dev mode (localhost)")
    console.print("  RESUMEFLOW_API_BASE=URL  Override API base URL")
...
def config_set(
    ...
    Settings are saved to ~/.resumeflow/config.json and used as defaults
    for the analyze command.
    
    Examples:
        # Set models for cloud analysis
        rf config-set --extraction-model gpt-4o-mini --synthesis-model gpt-4o
        
        # Configure local LLM (Ollama)
        rf config-set --local-api-url http://localhost:11434/v1 --local-api-key ollama
        rf config-set --extraction-model llama3.2 --synthesis-model llama3.2
        
        # Configure local LLM (LM Studio)
        rf config-set --local-api-url http://localhost:1234/v1 --local-api-key lm-studio
    ...
    if not any([extraction_model, synthesis_model, local_api_url, local_api_key]):
        ...
        console.print(f"  Local API Key:    {'[green]✓ set[/]' if llm_config['local_api_key'] else '[dim]not set[/]'}")
        console.print()
        console.print("Use [bold]rf config-set --help[/] to see available options.")
        return
